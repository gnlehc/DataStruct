...
{ // new height after insertion or deletion
...
{ // determine unbalanced left/right child -> determine linear/zig-zag pattern
  if (root->bf > 1)
  { // unbalanced left child contoh 2
    if (root->left->bf >= 0)
    {                           // right rotation
  ...
  { // unbalanced right child contoh -2
    if (root->right->bf <= 0)
    {                          // left rotation
      return leftRotate(root); // not zig zag pattern
    }
    else
    {                                         // right-left rotation -> zig zag pattern
...
int main()
{
  ...
  baseRoot = insertNode(baseRoot, 2);
  baseRoot = insertNode(baseRoot, 12);
  baseRoot = insertNode(baseRoot, 24);
  baseRoot = insertNode(baseRoot, 6);
  inOrder(baseRoot);
  // for(int i = 0; i < 9; i++) {
